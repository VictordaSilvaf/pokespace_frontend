...
collection. Paths below are relative to `public/assets/`.
...
index = json.loads((root / "sprite-meta" / "index.json").read_text())
meta  = json.loads((root / "sprite-meta" / "metadata.json").read_text())["things"]
...
`sprite-meta/index.json` is category-first. Invert it once for O(1) lookups:
...
```js
const index = await fetch("/assets/sprite-meta/index.json").then(r => r.json());
const meta  = await fetch("/assets/sprite-meta/metadata.json").then(r => r.json());

function resolve(assetId) {
  const category = Object.keys(index).find(c => index[c].includes(assetId));
  return {
    category,
    src: `/assets/sprites/${category}/${assetId}.png`,
...
```js
// In preload():
this.load.spritesheet("creature-40036", "/assets/sprites/creature/40036.png", {
  frameWidth: 32,
  frameHeight: 32,
});
...
credit line (see the pack NOTICE for ready-to-paste snippets).
